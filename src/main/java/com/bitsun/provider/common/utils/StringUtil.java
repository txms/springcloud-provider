...
public class StringUtil {
    ...
    public static List<Long> getIdList(String ids) {
        List<Long> idList;
        try {
            idList = Arrays.asList(ids.split(",")).stream().map(s -> NumberUtils.createLong(s.trim())).collect(Collectors.toList());
        } catch (Exception e) {
            throw e;
        }
        return idList;
    ...
    public static List<Integer> getIntList(String ids) {
        ...
        try {
            intList = Arrays.asList(ids.split(",")).stream().map(s -> NumberUtils.createInteger(s.trim())).collect(Collectors.toList());
